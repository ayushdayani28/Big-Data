...
class MyMRSimulator:
    ...
    def reduceTask(self, kvs, namenode_fromR): 
        ...
        for k, v in r.items():
            # print(k,v, 'reducejob')
            value = self.reduce(k,v)
            if value!=None:
                namenode_fromR.append(value)
    ...
    def runSystem(self): 
        ...
        #[TODO: DONE]#
        chunks = [self.data[x:x+chunkSize] if x+chunkSize<len(self.data) else self.data[x:] for x in range(0, len(self.data), chunkSize)]
        for chunk in chunks:
            p = Process(target=self.mapTask, args=(chunk,namenode_m2r,self.use_combiner))
            #maptask without prereduce takes around 15-20 min for test.csv with test.csv it takes less than 30 secs or so.()
            p.start()
            processes.append(p)
        ...
        # now = datetime.now()
        # current_time = now.strftime("%H:%M:%S")
        # print("Current Time =", current_time, 'map task completed')
        ...
        # now = datetime.now()
        # current_time = now.strftime("%H:%M:%S")
        # print("Current Time =", current_time,'reduce started')
        ...
        # now = datetime.now()
        # current_time = now.strftime("%H:%M:%S")
        # print("Current Time =", current_time,'reduce completed')
...
if __name__ == "__main__": #[Uncomment peices to test]
    ...
    # observation: we should use no of reducer different from 3 or multiples of 3 as task are assigned to processor 1 only after partitioning 1,10,100,1000,10000
    # or either iterate over datachunks_key to get a different value.
    mrObject.runSystem() 

## for 
# python MRSimulator_CSE545_sp22_v2_Dayani_114241407.py test_incomes.csv
# python MRSimulator_CSE545_sp22_v2_Dayani_114241407.py trial_incomes.csv
